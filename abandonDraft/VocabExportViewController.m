...
- (void)viewDidLoad
{
    ...
    UIButton *CancelSelection = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [CancelSelection setFrame:CGRectMake(0, 0, 160, 44)];
    [CancelSelection setTitle:@"cancel selection" forState:UIControlStateNormal];
    [CancelSelection addTarget:self action:@selector(didSelectCancel) forControlEvents:UIControlEventTouchDown];
    CharacterGridBox *buttonBox01 = [[CharacterGridBox alloc]initWithFrame:CGRectMake(0, 0, 160, 44)];
    [buttonBox01 addSubview:CancelSelection];
    [[grid boxes] addObject:buttonBox01];
    
    UIButton *FinishSelection = [UIButton buttonWithType:UIButtonTypeRoundedRect];
    [FinishSelection setFrame:CGRectMake(0, 0, 160, 44)];
    ...
    CharacterGridBox *buttonBox02 = [[CharacterGridBox alloc]initWithFrame:CGRectMake(0, 0, 160, 44)];
    ...
    for (id word in _wordList){
        ...
        wordTile.onTap = ^{
            if (!wordTile.selected){
                ...
                [[wordTile characterBoxText] setTextColor:[UIColor blackColor]];
